...
def prepare_environment():
    ...
    gfpgan_package = os.environ.get(
        'GFPGAN_PACKAGE',
        "git+https://ghproxy.com/github.com/TencentARC/GFPGAN.git@8d2447a2d918f8eba5a4a01463fd48e45126a379"
    )
    clip_package = os.environ.get(
        'CLIP_PACKAGE',
        "git+https://ghproxy.com/github.com/openai/CLIP.git@d50d76daa670286dd6cacf3bcd80b5e4823fc8e1"
    )
    openclip_package = os.environ.get(
        'OPENCLIP_PACKAGE',
        "git+https://ghproxy.com/github.com/mlfoundations/open_clip.git@bb6e834e9c70d9c27d0dc3ecedeebeaeb1ffad6b"
    )

    stable_diffusion_repo = os.environ.get(
        'STABLE_DIFFUSION_REPO',
        "https://ghproxy.com/github.com/Stability-AI/stablediffusion.git"
    )
    taming_transformers_repo = os.environ.get(
        'TAMING_TRANSFORMERS_REPO',
        "https://ghproxy.com/github.com/CompVis/taming-transformers.git"
    )
    k_diffusion_repo = os.environ.get(
        'K_DIFFUSION_REPO',
        'https://ghproxy.com/github.com/crowsonkb/k-diffusion.git'
    )
    codeformer_repo = os.environ.get(
        'CODEFORMER_REPO',
        'https://ghproxy.com/github.com/sczhou/CodeFormer.git'
    )
    blip_repo = os.environ.get(
        'BLIP_REPO',
        'https://ghproxy.com/github.com/salesforce/BLIP.git'
    )
    ...
    run_pip(  # '"...python.exe" -m pip install -r "requirements_versions.txt" --prefer-binary'
...
def tests(test_dir):
    ...
    with open(
            os.path.join(script_path, 'test/stdout.txt'),
            "w",
            encoding="utf8"
    ) as stdout, open(
        os.path.join(script_path, 'test/stderr.txt'),
        "w",
        encoding="utf8"
    ) as stderr:
