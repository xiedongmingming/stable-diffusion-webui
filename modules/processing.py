...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        latent_scale_mode = shared.latent_upscale_modes.get(
            self.hr_upscaler,
            None
        ) if self.hr_upscaler is not None else shared.latent_upscale_modes.get(
            shared.latent_upscale_default_mode,
            "nearest"
        )
        ...
        if self.enable_hr and latent_scale_mode is None:
            #
            assert len(
                [x for x in shared.sd_upscalers if x.name == self.hr_upscaler]
        ...
        if latent_scale_mode is not None:
            ...
            if getattr(self, "inpainting_mask_weight", shared.opts.inpainting_mask_weight) < 1.0:
                ...
                image_conditioning = self.img2img_image_conditioning(
                    decode_first_stage(self.sd_model, samples),
                    samples
                )
